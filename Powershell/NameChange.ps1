...
Get-ChildItem $Path -Filter *.mp4 -Recurse | ForEach-Object { $newname = $TextInfo.ToLower($_.BaseName) + ".mp4"; Rename-Item $_.FullName $newname }
Get-ChildItem $Path -Filter *.mp4 -Recurse | ForEach-Object { $newname = $TextInfo.ToTitleCase($_.BaseName) + ".mp4"; Rename-Item $_.FullName $newname }
