...
$DESID = $($csv | select -Property * | ? {$_.DESID.trim() -ieq "$env:USERNAME"}).DESID.trim()
$Eclient = $($csv | select -Property * | ? {$_.DESID.trim() -ieq "$env:USERNAME"}).EclientID.trim()
$EmailID = $($csv | select -Property * | ? {$_.DESID.trim() -ieq "$env:USERNAME"}).Email.trim()
$FirstName = $($csv | select -Property * | ? {$_.DESID -ieq "$env:USERNAME"}).FirstName.trim()
$LastName = $($csv | select -Property * | ? {$_.DESID -ieq "$env:USERNAME"}).LastName.trim()
